...
public class SearchView extends LinearLayout implements CollapsibleActionView {
	...
	public SearchView(Context context, AttributeSet attrs) {
		...
		if (mDropDownAnchor != null) {
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
				...
				mDropDownAnchor.getViewTreeObserver().addOnGlobalLayoutListener(
	...
	private void updateSubmitArea() {
		...
		if (isSubmitAreaEnabled()
				&& (mSubmitButton.getVisibility() == VISIBLE || mVoiceButton.getVisibility() == VISIBLE)) {
			visibility = VISIBLE;
		}
	...
	View.OnKeyListener mTextKeyListener = new View.OnKeyListener() {
		public boolean onKey(View v, int keyCode, KeyEvent event) {
			...
			if (DBG) {
				Log.d(LOG_TAG,
						"mTextListener.onKey(" + keyCode + "," + event + "), selection: "
	...
	private void updateSearchAutoComplete() {
		...
		if (mSearchable.getSuggestAuthority() != null) {
			...
			((SuggestionsAdapter) mSuggestionsAdapter)
					.setQueryRefinement(mQueryRefinement ? SuggestionsAdapter.REFINE_ALL
							: SuggestionsAdapter.REFINE_BY_ENTRY);
		}
	}
	...
	private void adjustDropDownSizeAndPosition() {
		if (mDropDownAnchor.getWidth() > 1) {
			...
			mQueryTextView.setDropDownWidth(mDropDownAnchor.getWidth() + dropDownPadding.left + dropDownPadding.right
					+ iconOffset - (anchorPadding));
		}
	}
