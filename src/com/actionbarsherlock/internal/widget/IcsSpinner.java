...
public class IcsSpinner extends IcsAbsSpinner implements OnClickListener {
	...
	private void setUpChild(View child) {
		...
		int childTop = mSpinnerPadding.top
				+ ((getMeasuredHeight() - mSpinnerPadding.bottom - mSpinnerPadding.top - child.getMeasuredHeight()) / 2);
	...
	private class DropdownPopup extends IcsListPopupWindow implements SpinnerPopup {
		...
		public void show() {
			...
			if (mDropDownWidth == WRAP_CONTENT) {
				...
				setContentWidth(Math.max(measureContentWidth((SpinnerAdapter) mAdapter, getBackground()), spinnerWidth
